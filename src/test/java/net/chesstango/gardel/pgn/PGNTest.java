...
public class PGNTest {
    ...
    public void test_createEPD() {
        ...
        PGNMove pgnMove = new PGNMove("e4", new TreeMap<>());
        pgnMove.putCommand(PGNMove.EVAL_COMMAND, "1000");

        EPD epd = pgn.createEPD(pgnMove, FEN.START_POSITION);

        assertEquals(FEN.START_POSITION.getPiecePlacement(), epd.getPiecePlacement());
        assertEquals(FEN.START_POSITION.getActiveColor(), epd.getActiveColor());
        assertEquals(FEN.START_POSITION.getCastingsAllowed(), epd.getCastingsAllowed());
        assertEquals(FEN.START_POSITION.getEnPassantSquare(), epd.getEnPassantSquare());
        assertEquals(FEN.START_POSITION.getHalfMoveClock(), epd.getHalfMoveClock());
        assertEquals(FEN.START_POSITION.getFullMoveClock(), epd.getFullMoveClock());

        assertEquals("e4", epd.getSuppliedMoveStr());
        assertEquals("1000", epd.getCentiPawnEvaluation());

        assertEquals("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - hmvc 0; fmvn 1; sm e4; ce 1000; c0 \"event='188c6255-5216-463a-bfc6-300b98a14ff9'\"; c1 \"site='LAPTOP-PTVVKHNB'\"; c2 \"date='2026.05.03'\"; c3 \"white='Tango'\"; c4 \"black='Oponent'\"; c5 \"result='0-1'\"; id \"1 w\";", epd.toString());
    }
